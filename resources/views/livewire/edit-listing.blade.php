<div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8 bg-white shadow-md rounded-lg mt-10">
    ...
    <form wire:submit.prevent="save" class="space-y-6">
        ...
        @if(count($existingImages) > 0)
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">ያሉ ፎቶዎች (Current Photos)</label>
                <div class="flex flex-wrap gap-3">
                    @foreach($existingImages as $index => $image)
                        <div class="relative group">
                            <img src="{{ asset('storage/' . $image) }}" class="w-24 h-24 object-cover rounded-lg border shadow-sm">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-indigo-500 transition">
            ...
            <label for="photo-upload" class="cursor-pointer">
                <div class="text-gray-500">
                    ...
                    <p class="text-xs text-gray-400">Up to 3 photos total</p>
                </div>
            </label>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('listing.mine') }}" wire:navigate
               class="px-6 py-4 bg-gray-200 text-gray-700 font-bold rounded-lg hover:bg-gray-300 transition text-center">
                ሰርዝ (Cancel)
            </a>
        </div>
    </form>
